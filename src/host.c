...
static void cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep) {
  ...
  if (len) {
    ...
    for (size_t i = 0; i < len; ++i) {
      cbuf_push(data_from_host, buf[i]);
    }
  }
}
...
void host_send(cbuf_t* data_from_xbee) {
  uint8_t buf[64];

  uint16_t frame_len = 0;
  size_t len = 0;
  while (cbuf_avail(data_from_xbee) && len < sizeof(buf)) {
    uint8_t b = cbuf_pop(data_from_xbee);
    if (len == 0 && b == 0x7e) {
      buf[len++] = b;
    } else if (len > 0 ) {
      buf[len++] = b;
